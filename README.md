...
An 8-bit ADC with 3.3V of full-scale voltage has a quantum equal to:

3.3V / 256 levels = 12.9mV
...
Let the plates have dimensions **w = 12 mm; l = 35 mm**, then the area
**S = 12 × 35 = 420 mm²**

Distance between them
**d = 3 mm**

Then the calculated electrical capacitance
**C = 1 pF**
...
The geometric dimensions (area) **S**, as well as the distance between the plates **d**, do not change. To change the capacitance, it remains to change the substance between the plates, as long as it is air **ε = 1**.

What do you think? relative dielectric constant of water? Sources show **ε = 81**.

---

### Capacitance calculation (dielectric: water)

Full immersion in water will increase the capacity by **81 times**.

The calculated capacitance **C** will no longer be **1 pF**, but **100 pF**.
...
3. Insert a project name and select **STM32Cube as Targeted Project Type**.

Setup STM32 project. Image by author.

4. After creating the project, a page will appear showing all the necessary features needed to configure the MCU.

STM32Cube device configuration.

The **STM32G071RB** has **2 ADCs (ADC1 and ADC2)** with a maximum sampling rate of **4MHz (0.25µs)** and up to **19 multiplexed channels**. Resolution of **12-bit** with a full-scale voltage range of up to **3.6V**.
...
The **Analog connector** of the development board is connected to pins:

**PA0, PA1, PA4, PB0, PC1, and PC0**
...
5. Open the **Pinout & Configuration** tab and click on **Analog → ADC1** in the Categories section.

6. In the **channel 1 (IN0)** dropdown menu select **Single-ended**.
...
![image](https://github.com/user-attachments/assets/abd6710c-d56a-4a19-9e2b-b49857a22534)
